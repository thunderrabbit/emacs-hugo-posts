...
(fset 'create-title-with-image-url-on-single-line
      (kmacro-lambda-form [?\C-a ?\C-k ?\C-x ?b ?* ?1 ?2 ?3 ?4 ?5 ?6 ?7 return ?\C-y return ?\C-y ?\C-e backspace backspace backspace backspace ?\C-r ?/ return right return escape ?x ?r ?e ?p ?l ?a tab ?s ?t ?r ?i ?n ?g return ?_ return ?  return ?\C-a up ?\C-k up ?\C-k ?\C-x ?b return ?< ?i ?m ?g ?  ?s ?r ?c ?= ?\" ?\C-y ?\" return ?a ?l ?t ?= ?\" ?\C-x ?b return down down ?\C-k ?\C-x ?k return ?\C-y ?\" return ?c ?l ?a ?s ?s ?= ?\" ?t ?i ?t ?l ?e ?\" ?  ?/ ?> ?\C-a down] 0 "%d"))
